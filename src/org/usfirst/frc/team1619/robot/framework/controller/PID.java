...
public class PID {
	...
	public int addProfile(double idleOutput, double f, double p, double i, double d, double maxOutput) {
		return this.addProfile(idleOutput, f, p, i, d, maxOutput / i, -1, -1, maxOutput);
	}

	public int addProfile(double idleOutput, double f, double p, double i, double d, double maxIntegral,
			double maxOutput) {
		return this.addProfile(idleOutput, f, p, i, d, maxIntegral, -maxIntegral, -1, maxOutput);
	}

	public int addProfile(double idleOutput, double f, double p, double i, double d, double maxIntegral,
			double integralRange, double maxOutput) {
		this.profiles.add(new Profile(idleOutput, f, p, i, d, maxIntegral, -maxIntegral, integralRange, maxOutput));
		return this.profiles.size() - 1;
	}

	public int addProfile(double idleOutput, double f, double p, double i, double d, double maxIntegral,
			double minIntegral, double integralRange, double maxOutput) {
		this.profiles.add(new Profile(idleOutput, f, p, i, d, maxIntegral, minIntegral, integralRange, maxOutput));
		return this.profiles.size() - 1;
	}
	...
	public double get(double measuredValue) {
		...
		if (insideIntegralRange) {
			...
			if (this.profile.maxIntegral != -1 && this.profile.minIntegral != -1) {
				if (this.integral < 0.0) {
					this.integral = Math.max(this.integral, this.profile.minIntegral);
				} else {
